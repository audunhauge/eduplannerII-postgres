...
exports.DummyHelper = function(app, timestamps) {
	var timestamps = {
		'content': 0
		, 'css': 0
	};
...
exports.add_overlay = function add_dummy_helper(app, req, locals) {
	var file = 'index';
			...
			if (err) {
				fs.stat(overlayCenteredPath, function (err, stats) {
					if (err) {
						...
						render(file+'-center.png', locals);
					}
				});
			} else {
				render(file+'.png', locals);
			}
		});
	});
	function render(overlay, locals) {
		locals['dummyHelperHtml'] = overlay ? '<div id="dummy-overlay" style="background-image: url(/img/overlays/'+overlay+');"></div>' : '';
		return locals;
	}
};
